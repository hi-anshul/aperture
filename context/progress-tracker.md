...
- **Phase 4: Connector SDK**
  ...
  - Lever connector (`packages/connectors/src/lever/`): `canHandle()` for `jobs.lever.co` / `api.lever.co` (global + EU); `fetch()` via public postings API (`api.lever.co/v0/postings/{site}?mode=json`) with pagination; recovers site from custom careers HTML embeds; worker parser/normalizer wired.
  - Ashby connector (`packages/connectors/src/ashby/`): `canHandle()` for `jobs.ashbyhq.com` / `api.ashbyhq.com`; `fetch()` via public job board API (`api.ashbyhq.com/posting-api/job-board/{board}?includeCompensation=true`); recovers board from custom careers HTML embeds; worker parser/normalizer wired.
  ...
  - Static HTML connector (`packages/connectors/src/static-html/`): HTTP fetch + configurable CSS-selector map (`cheerio`); JSON-LD `JobPosting` + common listing heuristics; selected by detected `platform === "static-html"` (not URL resolve).
  - React-rendered connector (`packages/connectors/src/react-rendered/`): Playwright/browser fetch via injected `browserFetchHtml`, reuses the same HTML parser; worker sync wires Fetch Engine `mode: "browser"`.
  - `createDefaultRegistry()` registers Greenhouse, Lever, Ashby, Workday, static-html, and (when `browserFetchHtml` is provided) react-rendered.
  - Fixture-based unit tests (17 Greenhouse + 17 Lever + 16 Ashby + 15 Workday + 6 static-html + 2 react-rendered + 23 platform-detector = 96 passing) — no live network required for CI.
...
- Lever connector live (`packages/connectors/src/lever/`): public postings API (`api.lever.co/v0/postings/{site}?mode=json`) with skip/limit pagination, global + EU hosts, site extraction from `jobs.lever.co` / API URLs and custom careers HTML embeds; parser + normalizer wired. Run `pnpm --filter @aperture/connectors test` and `pnpm --filter @aperture/worker test -- src/normalizer src/parser-engine`.
- Ashby connector live (`packages/connectors/src/ashby/`): public job board API (`api.ashbyhq.com/posting-api/job-board/{board}?includeCompensation=true`), board extraction from `jobs.ashbyhq.com` / API URLs and custom careers HTML embeds; parser + normalizer wired (including salary from `compensation.summaryComponents`). Run `pnpm --filter @aperture/connectors test` and `pnpm --filter @aperture/worker test -- src/normalizer src/parser-engine`.
- Static-html + react-rendered fallbacks live: shared CSS-selector HTML parser (`cheerio`); static-html uses HTTP fetch; react-rendered uses Fetch Engine Playwright (`mode: "browser"`) and the same parser. Worker sync registers both. Run `pnpm --filter @aperture/connectors test` and `pnpm --filter @aperture/worker test -- src/normalizer src/parser-engine`. Ensure `pnpm exec playwright install chromium` for SPA sites.
...
- Workday sync hardening: Node 24 / undici crashed the worker mid detail-fetch (`AssertionError` on paused HTTP/1 parser) leaving Visa syncs stuck at `running` with 0 jobs. Workday connector now uses userland `undici@8.8.0` with a dedicated Agent, lower detail concurrency, and full body buffering; root `pnpm-workspace.yaml` overrides undici to `^8.8.0`. Verified live Visa CXS fetch returns ~887 jobs without crashing.
- Country filter fix: was exact-match on `jobs.country` only, so "United States" / "India" missed most rows (Workday country often null; Greenhouse stored variants like "India Locations"). Filter now uses contains against `country` **or** `location`, with US/UK aliases. Greenhouse `City, ST` → `United States`; Workday derives country from `US - City` location prefixes when detail country is missing.
- Static-html parser hardening: link-less job cards (e.g. Flipkart `jobslist`) were dropped because `href` was required; titles in `h4`–`h6` were also missed. Parser now keeps cards without links (source URL = listing page, external id from title+location), expands title/job-item selectors, strips nested duplicate matches, and normalizes `Location :` prefixes. Verified live Flipkart sync → 10 jobs.
